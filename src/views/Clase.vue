...
const submit = handleSubmit(values => {
  console.log(values);
  fetch('http://127.0.0.1:8000/api/v1/Clase', {
    method: 'POST',
    headers: {
...
async function getClases() {
  try {
    ...
    itemClase.value = data;
    console.log(itemClase.value);
  } catch (error) {
    console.log(error);
    toast.error("Error : \nHa ocurrido un error en el servidor.", options);
  }
...
async function getMaestro() {
  try {
    ...
    const data = await response.json();
    itemMaestro.value = data.map(item => item.id);
  } catch (error) {
    console.log(error);
    toast.error("Error : \nHa ocurrido un error en el servidor.", options);
  }
...
async function getAulas() {
  try {
    const response = await fetch('http://127.0.0.1:8000/api/v1/Aula', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': localStorage.getItem("token")
    ...
    const data = await response.json();
    itemAula.value = data.map(item => item.id);
  } catch (error) {
    console.log(error);
    toast.error("Error : \nHa ocurrido un error en el servidor.", options);
  }
...
async function getGrupos() {
  try {
    ...
    const data = await response.json();
    itemGrupo.value = data.map(item => item.id);
  } catch (error) {
    console.log(error);
    toast.error("Error : \nHa ocurrido un error en el servidor.", options);
  }
          ...
          <div class="w-full">
            <div class="flex flex-col m-6 border-solid border-2 rounded-2xl pb-4 max-w-lg p-6">
              <form @submit.prevent="submit" class="flex flex-col justify-center items-center">
                <p class="text-3xl p-2 mb-4">Agregar Clase</p>
                <v-autocomplete v-model="grupo.value.value" :items="itemGrupo"
                  :error-messages="grupo.errorMessage.value" label="Grupo" variant="outlined"
                ...
                <v-autocomplete v-model="maestro.value.value" :items="itemMaestro"
                  ...
                  class="w-full mb-3"></v-autocomplete>
                <v-autocomplete v-model="aula.value.value" :items="itemAula"
                  :error-messages="aula.errorMessage.value" label="Aula" variant="outlined"
                  class="w-full mb-3"></v-autocomplete>
            ...
            <div class="flex flex-col m-6 border-solid border-2 rounded-2xl pb-4">
              <div class="flex mt-3 justify-between align-middle">
                <v-card-title>
                  <p class="text-3xl pt-2 pl-4">Clase</p>
                </v-card-title>
                <v-card-title>
              ...
              <v-data-table :headers="headers" :items="itemClase" :search="search" class="px-6"
