    ...
    <main>
        <div class="flex justify-center items-center h-screen w-screen bg-slate-200 text-slate-800 font-mono">
            <form @submit.prevent="service" class=" border-2 border-slate-800 p-10 flex flex-col min-w-[450px]">
                <div>
                    <h1 class="text-center text-4xl font-bold antialiased pb-2">Registro</h1>
                </div>
                ...
                <v-text-field v-model="password.value.value" :error-messages="password.errorMessage.value"
                    label="Contraseña" :append-inner-icon="visible ? 'mdi-eye-off' : 'mdi-eye'"
                    :type="visible ? 'text' : 'password'" variant="outlined" @click:append-inner.prevent="onClickAppendIcon"
                    ref="pwTxt" class="mb-1" placeholder="Introduzca su contraseña" />
                <div class="pb-2"></div>
                ...
                <a class=" text-slate-800 flex justify-center items-center pt-2 text-xs" href=""
                    @click.prevent="handleSignin()">
                    Iniciar sesión<v-icon icon="mdi-chevron-right" />
                </a>
            </form>
        </div>
    </main>
